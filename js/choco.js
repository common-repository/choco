...
choco.loadTinyMceForm = function () {
    ...
    jQuery.ajax({
        global: false,
        type: 'GET',
        url: choco.baseUrl + "/js/choco_tiny_mce.html",
        dataType: 'html'
...
choco.setClickEventTinyMceCancelButton = function () {
    jQuery("#choco_tiny_mce #tiny_mce_cancel,#choco_tiny_mce #tiny_mce_bg").unbind();
    // button cancel and click background
    jQuery("#choco_tiny_mce #tiny_mce_cancel,#choco_tiny_mce #tiny_mce_bg").click(function () {
        //jQuery("#choco_tiny_mce").remove();
        jQuery('#choco_tiny_mce #tiny_mce_bg').css('display', 'none');
        jQuery('#choco_tiny_mce #tiny_mce_input').css('display', 'none');
...
choco.getAllData = function (selectorsA) { // divide
    if (!selectorsA.length) {
        return;
    }

    /*
    // 1034
    for (var i = 0; i < selectorsA.length; i++) {
        console.log("!2" + selectorsA[i].selector + ' ' + selectorsA[i].selector_index);
    }
    */

    for (var i = 0; i < selectorsA.length; i++) {
        // console.log("!2" + selectorsA[i].selector + ' ' + selectorsA[i].selector_index);
        selectors = selectorsA[i];
...
choco.getSelectors = function () {
    ...
    var i = 0;
    var j = 0;
    var jmax = 100; // divide array
    selectors[i] = new Array();
    jQuery(choco.selector).each(function () {
        selectors[i][j++] = choco.getSelector(this);
        if (j === jmax) {
            i++;
            selectors[i] = new Array();
            j = 0;
        ...
        // 1034
        // console.log("sr: " + selectors[i-1]["selector"] + ", index: " + selectors[i-1]["selector_index"]);
    });
    return selectors;
};
...
choco.setClickEventTinyMceSaveButton = function () {
    // button save
    jQuery("#choco_tiny_mce #tiny_mce_save").unbind();

    jQuery("#choco_tiny_mce #tiny_mce_save").on('click', function () {
...
choco.ready = function () {
    ...
    var selectors = choco.getSelectors(); // divide
    /*
    for (var i = 0; i < selectors.length; i++) {
        console.log('! ' + selectors[i].selector + ' ' + selectors[i].selector_index);
    }
    */
    choco.getAllData(selectors); // divide
    ...
    jQuery(choco.selector).live('click', function (obj) {
        ...
        tinyMCE.get("choco_tinymce").execCommand('mceSetContent', false, html);
...
choco.mutationObserver = new MutationObserver(function (mutationRecords) {
    ...
    timer = setTimeout(function () {
        ...
        choco.getAllData(choco.getSelectors()); // divide
    }, 200);
});
